...
docker build -f "$ROOT_DIR/apps/gateway/Dockerfile" -t shardd-gateway:latest "$ROOT_DIR"

echo "==> Saving images to tarballs..."
docker save shardd-node:latest | gzip > /tmp/shardd-node.tar.gz
docker save shardd-gateway:latest | gzip > /tmp/shardd-gateway.tar.gz
...
rsync -avz --progress /tmp/shardd-gateway.tar.gz "$DEPLOY_USER@$DEPLOY_HOST:$REMOTE_DIR/"
...
ssh $SSH_OPTS "$DEPLOY_USER@$DEPLOY_HOST" "cd $REMOTE_DIR && gunzip -c shardd-node.tar.gz | docker load && gunzip -c shardd-gateway.tar.gz | docker load"
