...
type PersistedKeys = [OriginKey];
type PersistedCallback = dyn Fn(&PersistedKeys) + Send + Sync;
...
pub struct BatchWriter<S: StorageBackend> {
    ...
    on_persisted: Option<Box<PersistedCallback>>,
}
...
impl<S: StorageBackend> BatchWriter<S> {
    ...
    pub fn with_on_persisted(
        mut self,
        callback: impl Fn(&[OriginKey]) + Send + Sync + 'static,
    ) -> Self {
        ...
        match self.storage.insert_events_bulk(&events).await {
            Ok(inserted) => {
                ...
                if let Some(ref callback) = self.on_persisted {
                    let keys: Vec<OriginKey> = events.iter().map(|e| e.origin_key()).collect();
                    callback(&keys);
                }
            }
            Err(e) => {
...
mod tests {
    ...
    fn make_event(seq: u64) -> Event {
        Event {
            ...
            amount: 1,
            note: None,
            idempotency_nonce: format!("batch-test-nonce-{seq}"),
            void_ref: None,
            hold_amount: 0,
